...
float pmt_analyzer_simple(int runNum, float initialSig = 200.0, int run2 = 0, int run3 = 0, int run4 = 0, int run5 = 0,
	...
	TF1 *fit_func=new TF1("fit_func", poisson_model, 0, MAX_BIN, 8); 
	fit_func->SetLineColor(4); // Dark blue
	fit_func->SetNpx(2000);
	fit_func->SetLineWidth(2);
	// 6 parameters
	fit_func->SetParName(0, "Q0");
	fit_func->SetParName(1, "S0");
	fit_func->SetParName(2, "Q1");
	fit_func->SetParName(3, "S1");
	fit_func->SetParName(4, "mu");
	fit_func->SetParName(5, "inj");
	// These parameters are fixed and used for looping over 
	// each gaussian peak in the series of poissonian events.
	fit_func->SetParName(6, "MIN_PEAK");
	fit_func->SetParName(7, "MAX_PEAK");
	// Min PEAK must be at least one
	fit_func->FixParameter(6, 1);
	fit_func->FixParameter(7, 10);

	// Initialize the pedestal + rms, and signal + rms
	fit_func->SetParameter(0, initialPed);
	fit_func->SetParameter(1, 2.0);
	fit_func->SetParameter(2, initialSig);
	fit_func->SetParameter(3, sqrt(initialSig));
	fit_func->SetParameter(4, 1.0);
	fit_func->FixParameter(5, 0.0);
	
	// Set bounds on parameters where necessary
	fit_func->SetParLimits(5, 0.0, 1.0); // Proportion of non-real events
	...
	for ( int bb = 1; bb < 10; bb++) {
		...
		fit_func->GetParameters(back);
		back[6] = (double)(bb);
        	fis_from_fit_pe[bb] = new TF1(Form("pe_fit_%d", bb), poisson_model_pe, 0, MAX_BIN, 8);
	...
	TF1 *fis_from_fit_bg = new TF1("fis_from_fit_bg", poisson_model_bg, 0, MAX_BIN, 8);
	...
	// Get output values
	Double_t pedout = back[0];
	Double_t pedrmsout = back[1];
	Double_t sigout = back[2];
	Double_t sigrmsout = back[3];
	Double_t muout = back[4];
	Double_t injout = back[5];
	Double_t pedouterr = fit_func->GetParError(0);
	Double_t pedrmsouterr = fit_func->GetParError(1);
	Double_t sigouterr = fit_func->GetParError(2);
	Double_t sigrmsouterr = fit_func->GetParError(3);
	Double_t muouterr = fit_func->GetParError(4);
	Double_t injouterr = fit_func->GetParError(5);
